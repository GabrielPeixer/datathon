...
## Checklist do demo day

- [x] Repositório organizado com código e dependências (`requirements.txt`).
- [x] Notebook de EDA com a base Kaggle limpa e referenciada.
- [x] Modelo Baseline e Modelo Adaptativo implementados e comparados.
- [x] Golden Set com 5 casos de teste e recomendações geradas (notebook + README).
- [x] Código executável que retorna a predição (API FastAPI + notebook).
- [x] README com link da base, parágrafo de infraestrutura AWS e instruções de execução.
- [x] Tracking de experimentos via MLflow.
- [x] Testes automatizados do pipeline, políticas e API.
- [ ] Vídeo pitch de até 5 minutos (gravar demonstrando `src.api` respondendo).
